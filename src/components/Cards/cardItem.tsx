      ...
      <div className="flex flex-col gap-4 md:min-h-[500px]">
        <div data-placeholder="true" data-ratio="16:9">
          <Image
            ...
            width={600}
            height={270}
            className="rounded-lg object-cover w-full h-auto max-h-[270px]"
            unoptimized
          />
        </div>
        <p className="text-slate-400 text-xs font-medium font-['Inter'] leading-tight">
          {formatDate(blogs?.[0]?.createdDate ?? "", "dd MMM · HH:mm 'UTC'")}
        </p>
        <div className="flex flex-col gap-3">
          <h3 className="text-white text-xl font-bold font-['Inter'] uppercase leading-loose line-clamp-2">
            {blogs?.[0]?.title}
          </h3>
          <p className="text-slate-400 text-sm font-medium font-['Inter'] leading-snug line-clamp-2">
            {blogs?.[0]?.description}
          </p>
