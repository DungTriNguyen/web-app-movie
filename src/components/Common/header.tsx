...
const Header = () => {
  ...
  return (
    <header
      className={`${
        isScrolled && !menuOpen
          ? "bg-gradient-to-b from-black/90 to-black/0 backdrop-blur-xs"
          : "bg-transparent"
      ...
      <div className="min-h-20 md:py-6 px-5 md:px-20 flex gap-2.5 justify-between items-center">
        ...
        <div className="flex items-center gap-2">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <a
                  href={link.href}
                  key={link.href}
                  className={`min-h-9 px-1 inline-flex text-sm justify-center items-center gap-2.5 cursor-pointer ${
                    isActive ? "border-b-2 border-white" : ""
                  }`}
                >
                  <span className="text-right justify-center text-white text-sm font-medium font-['Inter'] leading-snug">
                    {link.label}
                  </span>
                </a>
              );
          ...
          <div className="hidden md:flex items-center gap-1">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  className="flex items-center gap-1 bg-transparent hover:bg-transparent focus:bg-transparent cursor-pointer"
                  variant="ghost"
                  size="sm"
                >
                  <span>
                    {languages
                      .find((lang) => pathname.startsWith(`/${lang.code}`))
                      ?.code.toUpperCase() || "EN"}
                  </span>
                      ...
                      <div className="flex w-full justify-between items-center">
                        {currentLocale === lang.code ? (
                          <>
                            ...
                            <span className="text-purple-600 self-stretch text-right justify-start text-sm font-normal font-['Inter'] leading-tight">
                              {lang.label}
                            </span>
                          </>
                        ) : (
                          <>
                            <span className="w-5" />
                            <span className="text-slate-400 self-stretch text-right justify-start text-sm font-normal font-['Inter'] leading-tight">
                              {lang.label}
                            </span>
                          </>
                        )}
          ...
          <a href={NEXT_PUBLIC_APP_URL}>
            <Button
              className="bg-primary hidden md:flex gap-2 justify-center items-center px-4 min-h-12 cursor-pointer"
              size="sm"
              variant="default"
            >
              {t("get started")}
        ...
        <div className="md:hidden flex items-center gap-4">
          <div className="max-w-[400px] md:hidden">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  className="flex items-center gap-1 bg-transparent hover:bg-transparent focus:bg-transparent cursor-pointer w-full justify-between"
                  variant="ghost"
                  size="sm"
                >
                  <span>
          ...
          <div
            className={`fixed inset-0 z-40 transition-opacity duration-300 bg-accent/90 ${
          ...
          >
            <div className="flex flex-col items-end justify-start h-full gap-12 px-4 pt-30">
              ...
              >
                <Button
                  className="bg-primary flex md:hidden gap-2 justify-center items-center px-4 min-h-12 mb-2"
                  size="sm"
                  variant="default"
                >
                  {t("get started")}
