...
interface GetListCountriesMovieServiceKphimOptions {
  type_list: string;
  page: number;
  sort_field: string;
  sort_type: string;
...
export async function getListCountriesMovieServiceKphim(
  options: GetListCountriesMovieServiceKphimOptions
): Promise<BaseServiceResponse<any>> {
  try {
    const response = await axiosInstance.get<any>(
      `${NEXT_PUBLIC_API_URL_OP2}/v1/api/quoc-gia/${options.type_list}`,
      {
        params: {
          ...options,
        },
